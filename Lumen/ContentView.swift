...
struct ContentView: View {
    ...
    // Test function for data parsing
    private func testDataParsing() {
        Task {
            do {
                let cozeAPI = CozeAPIService.shared
                let photos = try await cozeAPI.getDigitalTwinPhotosByYear("2024")
    ...
    var body: some View {
        HStack(spacing: 16) {
            ...
            ZStack {
                ...
                Image(systemName: getSystemIcon(for: item.imageName))
        ...
        .shadow(color: .blue.opacity(0.1), radius: 20, x: 0, y: 0)
    }
}
